...
monitor.I=function(ob_boin,ob_data,ndose,sigma.spike,sigma.slab,ndraw,nchain,burn,thin,seed_number){
  ...
  for(j in 1:ndose){
    ...
    n1=length(ob_boin[ob_boin$dose==j,'yI'])
    
    y1=sum(ob_boin[ob_boin$dose==j,'yI'])
    
    n2=length(ob_data[ob_data$dose==j,'yI'])
    
    y2=sum(ob_data[ob_data$dose==j,'yI'])
  ...
  for(i in 1:ndraw){pI.draw.pava[i,]=pava(y=pI.draw[i,],w=w)}
...
monitor.T=function(ob_boin,ob_data,ndose,immunity,sigma.spike,sigma.slab,ndraw,nchain,burn,thin,seed_number){
  ...
  for(i in 0:1){
    ...
    for(j in 1:ndose){
      ...
      n1=length(ob_boin[ob_boin$yI==i & ob_boin$dose==j,'yT'])
      
      y1=sum(ob_boin[ob_boin$yI==i & ob_boin$dose==j,'yT'])
      
      n2=length(ob_data[ob_data$yI==i & ob_data$dose==j,'yT'])
      
      y2=sum(ob_data[ob_data$yI==i & ob_data$dose==j,'yT'])
  ...
  for(i in 1:ndraw){
    ...
    if(ndose==1){d=as.matrix(pava(y=d,w=w))}else{d=biviso(y=d,w=w)}
...
monitor.E=function(ob_boin,ob_data,ndose,immunity,sigma.spike,sigma.slab,ndraw,nchain,burn,thin,seed_number){
  ...
  for(i in 0:1){
    ...
    for(j in 1:ndose){
      ...
      n1=length(ob_boin[ob_boin$yI==i & ob_boin$dose==j,'yE'])
      
      y1=sum(ob_boin[ob_boin$yI==i & ob_boin$dose==j,'yE'])
      
      n2=length(ob_data[ob_data$yI==i & ob_data$dose==j,'yE'])
      
      y2=sum(ob_data[ob_data$yI==i & ob_data$dose==j,'yE'])
...
get.mcmc=function(n1,y1,n2,y2,sigma.spike,sigma.slab,nchain,ndraw,burn,thin,seed_number){
  ...
  model='model{
    ...
    logit(p1) <- logit(p2) + delta

    p2 ~ dbeta(1, 1)
    
    delta ~ dnorm(0, tau[omega + 1])

    omega ~ dbern(0.5)

    tau[1] <- pow(sigma.slab, -2)
    
    tau[2] <- pow(sigma.spike, -2)
  ...
  update(model,n.iter=burn,progress.bar='none')
  
  sample=coda.samples(model,variable.names=c('p2'),n.iter=niter,thin=thin,progress.bar='none')
  
  sample=unlist(sample)
  
  sample=sample[1:ndraw]
...
get.oc=function(pI.true,pT0.true,pT1.true,pE0.true,pE1.true,rho0,rho1,
                      ...
                      phi.pT,phi.pE,cf.pT,cf.pE,ndraw,nstage,nsample,utable,
                      sigma.spike,sigma.slab,nchain,burn,thin,seed_number){
