...
monitor.T=function(ob_data,ndose,ndraw,immunity){
  ...
  pI.draw=as.matrix(immunity$pI.draw)
  ...
  pT.draw=matrix(0,nrow=ndraw,ncol=ndose)
  ...
  pT.draw=pI.draw*pT1.draw+(1-pI.draw)*pT0.draw
  
  return(list(pT0.draw=pT0.draw,pT1.draw=pT1.draw,pT.draw=pT.draw))
...
monitor.E=function(ob_data,ndose,ndraw,immunity){
  ...
  pE.draw=matrix(0,nrow=ndraw,ncol=ndose)
  ...
  pE.draw=pI.draw*pE1.draw+(1-pI.draw)*pE0.draw
  
  return(list(pE0.draw=pE0.draw,pE1.draw=pE1.draw,pE.draw=pE.draw))
}
...
monitor.U=function(immunity,toxicity,efficacy,utable){
  ...
  pI=as.matrix(immunity$pI.draw)
  
  pT0=as.matrix(toxicity$pT0.draw)
  
  pT1=as.matrix(toxicity$pT1.draw)
  
  pE0=as.matrix(efficacy$pE0.draw)
  
  pE1=as.matrix(efficacy$pE1.draw)
...
get.adm.set=function(ndose,phi.pT,phi.pE,cf.pT,cf.pE,toxicity,efficacy){
  ...
  pT.draw=as.matrix(toxicity$pT.draw)
  
  pE.draw=as.matrix(efficacy$pE.draw)
  ...
  for(j in 1:ndose){
    ...
    if(mean(pT.draw[,j]>phi.pT)<cf.pT){adm.toxicity[j]=1}
    
    if(mean(pE.draw[,j]<phi.pE)<cf.pE){adm.efficacy[j]=1}
  }
